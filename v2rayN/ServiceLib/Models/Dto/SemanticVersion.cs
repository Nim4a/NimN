...
public class SemanticVersion : IEquatable<SemanticVersion>, IComparable
{
    ...
    private static int ComparePreRelease(string? left, string? right)
    {
        ...
        if (leftRevision.HasValue || rightRevision.HasValue)
        {
            if (leftRevision.HasValue && rightRevision.HasValue)
            {
                return leftRevision.Value.CompareTo(rightRevision.Value);
            }
            return leftRevision.HasValue ? 1 : -1;
        }
    ...
    private static int? GetNimNRevision(string? prerelease)
    {
        if (prerelease is { Length: > 1 }
            && prerelease[0] == 'P'
            && prerelease.AsSpan(1).IndexOfAnyExceptInRange('0', '9') < 0
            && int.TryParse(prerelease.AsSpan(1), out var revision))
        {
            return revision;
        }
        return null;
